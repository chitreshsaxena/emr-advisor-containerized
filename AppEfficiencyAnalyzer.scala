...
class AppEfficiencyAnalyzer extends AppAnalyzer with Logging {
  ...
  override def analyze(appContext: AppContext, startTime: Long, endTime: Long, options: Map[String, String]): Unit = {
    ...
    val perfectJobTime = if (totalCores != 0) inJobComputeMillisUsed / totalCores else 0
    ...
    val jobDurationInSeconds = Duration(jobTime, TimeUnit.MILLISECONDS).toSeconds
    val executorsTasksPerSecond: Double =
      if (jobDurationInSeconds > 0)
        appContext.appMetrics.getTotalTasks.toDouble / jobDurationInSeconds
      else
        0.0

    val coreMemoryRatios = appContext.appSparkExecutors.getOnlyExecutors.map(_._2.getCoreMemoryRatio).toList
    val executorCoreMemoryRatio: Long = if (coreMemoryRatios.nonEmpty) coreMemoryRatios.max else 0L
